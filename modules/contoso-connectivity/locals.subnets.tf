locals {
  bastion_subnets = { for key, value in var.hub_virtual_networks : key => {
    bastion = {
      ...
      network_security_group = try(var.subnet_network_security_group_ids[key]["bastion"], null) == null ? null : {
        id = var.subnet_network_security_group_ids[key]["bastion"]
      }
      route_table = {
        id                           = null
  ...
  private_dns_resolver_subnets = { for key, value in var.hub_virtual_networks : key => {
    dns_resolver = {
      ...
      network_security_group = try(var.subnet_network_security_group_ids[key]["dns_resolver"], null) == null ? null : {
        id = var.subnet_network_security_group_ids[key]["dns_resolver"]
      }
      route_table = {
        id                           = null
        assign_generated_route_table = false
